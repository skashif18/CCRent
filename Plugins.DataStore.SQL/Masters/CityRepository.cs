namespace Plugins.DataStore.SQL.Masters
{
    ...
    public class CityRepository : ICityRepository
    {
        ...
        public CityRepository(CarRentContext _db)
        => db = _db;
        public Response Create(SysCity model)
        {
            try
            {
                db.Add(model);
        ...
        public IEnumerable<SysCity> GetAll()
        => db.SysCities;

        public SysCity GetById(int id)
        => db.SysCities.Where(m => m.Id == id).FirstOrDefault();

        public IEnumerable<SysCity> GetCitiesByCountryId(int CountryId)
        => db.SysCities.Where(c => c.CountryId == CountryId);
